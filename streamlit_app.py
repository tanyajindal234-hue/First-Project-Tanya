...
# Page config
st.set_page_config(page_title="Zomato AI Recommendation", page_icon="🍴", layout="wide")

# CSS Styling
st.markdown("""
<style>
.main {background-color: #f8f8f8;}
.stButton>button {background-color: #e23744; color: white; border-radius: 8px; border: none; font-weight: 600;}
...
try:
    ...
    if df is None and not st.session_state.data_initialized:
        st.info("👋 Dataset not found. Please run preprocessing.")
        ...
        with col1:
            if st.button("🚀 Initialize Dataset", use_container_width=True):
                ...
                path = Path("data/processed/zomato_clean.parquet")
                with st.status("Downloading and processing data..."):
        ...
        selected_cuisines = st.sidebar.multiselect("Preferred Cuisines", sorted(all_cuisines))
                ...
                if not candidates:
                    ...
                    if use_ai:
                        try:
                            ...
                            display_results = generate_llm_recommendations(prefs, candidates, max_results=5)
                        except Exception:
